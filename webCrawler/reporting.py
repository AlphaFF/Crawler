...
# -*- coding: utf-8 -*-
# @Author: wangfeng1
# @Date:   2019-02-14 16:01:43
# @Email: liushahedi@gmail.com
# @Last Modified by:   wangfeng1
# @Last Modified time: 2019-02-15 09:50:32
...
def url_report(stat, stats, file=None):
    ...
    if stat.exception:
        ...
        stats.add('redirect')
        print(stat.url, stat.status, 'redirect', stat.next_url, fie=file)
    elif stat.content_type == 'text/html':
        stats.add('html')
        stats.add('html_bytes', stat.size)
        print(stat.url, stat.status, stat.content_type, stat.encoding, stat.size, '%d/%d' % (stat.num_new_urls, stat.num_urls), file=file)
    else:
        if stat.status == 200:
            stats.add('other')
        ...
        print(stat.url, stat.status, stat.content_type, stat.encoding, stat.size, file=file)
